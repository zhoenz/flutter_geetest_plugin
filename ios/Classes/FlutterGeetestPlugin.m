...
- (void)gtCaptcha:(GT3CaptchaManager *)manager willSendSecondaryCaptchaRequest:(NSURLRequest *)originalRequest withReplacedRequest:(void (^)(NSMutableURLRequest *))replacedRequest {
    ...
    for (int i=0; i<arr.count-1; i++) {
        ...
        if([cur rangeOfString:@"="].location>0){
            ...
            [dic setObject: curStrArr[1] forKey:curStrArr[0]];
        }
    }
    NSError*parseError =nil;
    NSData*newData = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:&parseError];
    [mReuqest setHTTPBody: newData];
    NSString *contentType = [NSString stringWithFormat:@"application/json"];
    [mReuqest setValue:contentType forHTTPHeaderField:@"Content-Type"];
    NSLog(@"\n jsondatais: %@",[[NSString alloc]initWithData:newData encoding:NSUTF8StringEncoding]);
